...
exclude_no_change <- function(exonsAndIntrons,replicates_condition1,replicates_condition2,fasta,threshold_junc=10,threshold_ratio=1){
  transcripts <- unique(exonsAndIntrons$Parent)
  ...
  if(len_tr > 0){
    for(i in 1:len_tr){
      ...
      if(length(junc_ranges) > 0){
        ...
        first_empty_space <- unlist(lapply(names(fasta),function(x){strsplit(as.character(x)," ")[[1]][1]}))
        chromosome_sequence <- fasta[which(first_empty_space == chromosome),]
      ...
      if(!any(is.na(logFC_exons))){
        ## logFC have different signs (+ and -)
        if(sum(sign(logFC_exons)) != length(logFC_exons)) as_genes <- c(as_genes,current_transcript$gene[1])
        ## high difference of exon expression
        if(sum(sign(logFC_exons)) == length(logFC_exons) && any(diff(logFC_exons) > threshold_ratio)) as_genes <- c(as_genes,current_transcript$gene[1]) 
      }
      ...
      if(!(any(is.na(introns$logFoldChange_junctions))||(any(is.na(logFC_introns))))){  
        ...
        if(sum(sign(logFC_introns)) != sum(sign(introns$logFoldChange_junctions))){ as_genes <- c(as_genes,current_transcript$gene[1])
          ...
          if(!is.na(difff) && any(difff > threshold_ratio)) as_genes <- c(as_genes,current_transcript$gene[1])}
      } 
    }
  }
