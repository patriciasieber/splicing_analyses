...
exclude_no_change <- function(exonsAndIntrons,replicates_condition1,replicates_condition2,fasta,threshold_junc=10,threshold_ratio=1){
   ...
   if(len_tr > 0){
      ...
      if(as.character(strand(current_transcript)[1]) == "+"){ 
      ...
      # also possible: GC..AG, AT..AC (just a low percentage but possible)
      if(length(junc_ranges) > 0){
        ...
        if((length(chromosome_sequence) != 1) & (species == "Hcapsulatum" | species == "Cneoformans")){
          real_chromosome_sequence <- unlist(lapply(names(chromosome_sequence),function(z){strsplit(z," ")[[1]][1]}))
          chromosome_sequence <- chromosome_sequence[real_chromosome_sequence == chromosome,]
        } 
        if(length(chromosome_sequence) != 1) chromosome_sequence <- fasta[names(fasta) == chromosome,]
        ...
        for(k in 1:length(junc_ranges)){
          ...
          if(as.character(strand == "+")){
            if((substr(seq_junc,1,2) == "GT" & substr(seq_junc,width_junc-1,width_junc) == "AG") | (substr(seq_junc,1,2) == "GC" & substr(seq_junc,width_junc-1,width_junc) == "AG") | (substr(seq_junc,1,2) == "AT" & substr(seq_junc,width_junc-1,width_junc) == "AC")){
              valid_junc_ranges <- c(valid_junc_ranges,junc_ranges[k])
            }
          }else{
            if((substr(seq_junc,1,2) == "CT" & substr(seq_junc,width_junc-1,width_junc) == "AC") | (substr(seq_junc,1,2) == "CT" & substr(seq_junc,width_junc-1,width_junc) == "GC") | (substr(seq_junc,1,2) == "GT" & substr(seq_junc,width_junc-1,width_junc) == "AT")){ ## reverse complement
              valid_junc_ranges <- c(valid_junc_ranges,junc_ranges[k])
            }
          }
        }
      ...
      exon_ratio <- foldchange(exons$tpm1,exons$tpm2)
      ...
      ## to identify IR
      intron_ratio <- foldchange(introns$tpm1,introns$tpm2)
